...
class ConfigManager:
    ...
    def _validate_config(self, config: ExperimentConfig):
        ...
        relora_cfg = config.get("relora")
        lora_cfg = config.get("lora")

        if relora_cfg is not None:
            if relora_cfg.rank <= 0:
                raise ValueError("ReLoRA rank must be positive")
            if relora_cfg.alpha <= 0:
            ...
            if relora_cfg.adapter_weight_decay < 0:
                raise ValueError("ReLoRA adapter_weight_decay must be non-negative")
            ...
            raise ValueError("Configuration must include either a 'lora' or 'relora' section")
...
if __name__ == "__main__":
    ...
    elif config.get("relora") is not None:
        print(f"ReLoRA rank: {config.relora.rank}")
    ...
    if config_with_overrides.get("lora") is not None:
