...
def _resolve_split_directory(root: str, requested_split: str) -> Tuple[str, str]:
    """Resolve the on-disk directory for a given logical split name."""
    ...
    raise RuntimeError(
        f"Dataset split directory for '{requested_split}' not found. Searched: {searched}"
    )
...
class ImageNetDataset(Dataset):
    ...
    def __init__(
        ...
        self.split_dir, self.actual_split = _resolve_split_directory(root, split)
        ...
        print(
            f"Found {len(self.samples)} images in {len(self.classes)} classes for"
            f" {self.actual_split} split (requested '{split}')"
        )
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        try:
            ...
            print(f"Warning: Could not load image {path}: {str(e)}")
...
def create_imagenet_dataloaders(
    ...
    val_dataset = ImageNetDataset(root=data_root, split="val", transform=val_transform)
...
if __name__ == "__main__":
    ...
    data_root = "/speedy/datasets/ImageNet"

    train_loader, val_loader = create_imagenet_dataloaders(
        data_root=data_root, batch_size=16, num_workers=2
    )
    ...
    for batch_idx, (images, targets) in enumerate(train_loader):
        print(
            f"Batch {batch_idx}: images shape {images.shape}, targets shape {targets.shape}"
        )
